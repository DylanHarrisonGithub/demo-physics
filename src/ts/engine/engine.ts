import Edge2D from "./augmented-edge";
import Vector2D from "./augmented-vector";
import Ball from "./augmented-ball";
import { UIController } from "./ui";

export interface Actor {
  attributes: { [key: string]: string | number | boolean | null },
  move: (dt: number) => void,
  applyForce: (f: Vector2D) => void,
...
export class Engine {
  ...
  state: {
    numElements: number,
    actors: Actor[],
    elements: Array<Edge2D | Vector2D>
  }

  readonly minTime: number = 0.000001; //Number.MIN_VALUE;

  constructor(actors: Actor[], elements: Array<Edge2D | Vector2D>) { 
    let numElements = 0;
  ...
  getElements(): Array<Edge2D | Vector2D> { return this.state.elements; }
  pushActor(a: Actor) { a.attributes.id = this.state.numElements++; this.state.actors.push(a); }
  pushElement(e: Edge2D | Vector2D) { e.attributes.id = this.state.numElements++; this.state.elements.push(e); }
  popActorById(id: string): Actor | undefined { return this.state.actors.filter(a => a.attributes.id === id)[0]; }
  popElementById(id: string): Edge2D | Vector2D | undefined { return this.state.elements.filter(e => e.attributes.id === id)[0]; }
  ...
  moveActors(dt: number): UIController[] {
  
    let nextCollisions = this.determineNextCollisions();
    let additionalRenders: UIController[] = [];
    ...
    while (nextCollisions[0].t < dt && nextCollisions[0].t > 0) {
      ...
      nextCollisions.forEach(nextCollision => {
        if (nextCollision.Object2 instanceof Ball) {
          let vs = (<Ball>nextCollision.Object1).deflectWithBall(nextCollision.Object2);
          additionalRenders.push(...vs);
        } else {
